const dao = require('./mst_workflow_steps_role_user.dao');
...
module.exports.getRole = async (data) => {
    try {
        let resp = await dao.getRole(data);
        if (resp) {
            return { success: true, status: util.statusCode.SUCCESS, message: '', response: await util.encryptResponse(resp) }
        } else {
            return { success: false, status: util.statusCode.INTERNAL, message: 'Internal server error', response: null }
...
module.exports.getWorkflowUser = async (data) => {
    try {
        let resp = await dao.getWorkflowUser(data);
        if (resp) {
            return { success: true, status: util.statusCode.SUCCESS, message: '', response: await util.encryptResponse(resp) }
        } else {
            return { success: false, status: util.statusCode.INTERNAL, message: 'Internal server error', response: null }
...
module.exports.add = async (data) => {
    try {
        ...
        if (!isExists) {
            ...
            return { success: false, status: util.statusCode.NAME_EXISTS, message: 'Workflow step with same details already exists', response: null }
        }
