const dao 			        =    require('./mst_entity.dao');
...
module.exports.add = async (data) => {
    try{
        let isExists = await dao.checkName(data.reraid, data.entitydesc, 0);
        if (!isExists) {
...
module.exports.update = async (data) => {
    try{
        let isExists = await dao.checkName(data.reraid, data.entitydesc, data.entityid);
        if (!isExists) {
