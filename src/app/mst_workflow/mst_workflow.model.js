const dao = require('./mst_workflow.dao');
    ...
    try {
        ...
        if (!isExists) {
            if (data.defaultflg == 1) {
                const resp1 = await dao.updateDefaultFlg(data);
                if (resp1) {
            ...
            return { success: false, status: util.statusCode.NAME_EXISTS, message: 'Workflow details with same details already exists', response: null }
        }
    } catch (e) {
        return { success: false, status: util.statusCode.SOME_ERROR_OCCURRED, message: 'Some error occurred', response: null }
    }
...
module.exports.update = async (data) => {
    try {
        ...
        if (!isExists) {
            if (data.defaultflg == 1) {
                util.createLog(">>>>>>>", data.defaultflg)
                const resp1 = await dao.updateDefaultFlg(data);
                if (resp1) {
