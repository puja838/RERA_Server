...
module.exports.getList  = async(data) => {
    try {
        const sql = "SELECT a.entityid, b.entitydesc, a.stepid, ms.stepdesc, mt.entitytypedesc, a.entitytypeid" +
            " from mst_entitytype_step_fields a, mst_entity b, mst_steps ms, mst_entity_type mt" +
...
module.exports.getFieldList  = async(data) => {
    try {
        const sql = "SELECT a.id, a.sequenceno as sequence, a.isrequired as isRequired, a.approvalrequired as isApproval, a.fieldid as fieldId, a.groupid as groupId, a.rowname, a.fieldwidth as width, a.fontsize as size, IF(a.fieldid IS NOT NULL, 'field', 'group') as fieldType, b.fielddesc, b.fielddisplaydesc, c.groupname, a.parentfieldid as parentFieldId, a.parentfieldvalue as parentValue, b.fielddesc as parentfielddesc\n" +
            "FROM mst_entitytype_step_fields a LEFT JOIN mst_fields b ON a.fieldid = b.fieldid LEFT JOIN mst_fields_group c ON a.groupid = c.groupid LEFT JOIN mst_fields mf ON a.parentfieldid = mf.fieldid \n" +
...
module.exports.add  = async(data) => {
    try {
        ...
        for (const obj of data.fieldList) {
            const sql = "INSERT INTO mst_entitytype_step_fields (sequenceno, isrequired, approvalrequired, reraid, entityid, entitytypeid, stepid, fieldid, groupid, rowname, fieldwidth, fontsize, parentfieldid, parentfieldvalue)" +
                " values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
module.exports.checkModuleExistance = async (data) => {
    try {
        ...
        const sql = "SELECT id FROM mst_entitytype_step_fields WHERE deleted='0' AND reraid = ? AND stepid = ? AND id != ?" + where
        const [result] = await  readConn.query(sql,[data.reraId, data.stepId, data.id]);
...
module.exports.update  = async(data) => {
    try {
        ...
        const sql = "UPDATE mst_entitytype_step_fields SET isrequired = ?, approvalrequired = ?, fieldid = ?, groupid = ?,rowname = ?, fieldwidth = ?, fontsize = ?, parentfieldid = ?, parentfieldvalue = ?, sequenceno = ? WHERE id = ?";
...
module.exports.delete  = async(data) => {
    try {
        const sql = "UPDATE mst_entitytype_step_fields SET deleted = '1' WHERE id = ?";
